...
public class UploadProfilePictureHandler(IGenericRepository<User> genericRepository, IFileStorageService fileStorageService,IUnitOfWork unitOfWork ,ILogger<UploadProfilePictureHandler> logger) : IRequestHandler<UploadProfilePictureCommand, Result<string>>
{
    public async Task<Result<string>> Handle(UploadProfilePictureCommand request, CancellationToken cancellationToken)
    {
        var user = await genericRepository.GetByUserIdAsync(request.UserId, cancellationToken);
        ...
        user.SetProfilePicture(url.Value, url.Value);
